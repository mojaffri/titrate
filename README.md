...
```mermaid
flowchart LR
    GH["GitHub Actions<br/>tests + OIDC"] --> ECR["Amazon ECR<br/>immutable image"]
    ECR --> AR["AWS App Runner<br/>FastAPI container"]
    AR --> API["Health, prediction,<br/>metrics and drift APIs"]
    UI["Streamlit Model Lab"] --> API
    CFN["CloudFormation"] --> ECR
    CFN --> AR
    BUDGET["AWS Budget alerts"] -. cost guardrail .-> AR
```

AWS is optional: all application, container, documentation and infrastructure checks run locally or in GitHub Actions without provisioning cloud resources. The deployment defaults to one maximum App Runner instance, keeps only five container images, leaves billable CloudWatch alarms disabled, and includes an explicitly confirmed teardown workflow. The repository demonstrates a validated cloud design without claiming that an AWS service is permanently running.
